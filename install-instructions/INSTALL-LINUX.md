...
If `curl` is missing: `wget -qO- https://astral.sh/uv/install.sh | sh`.

Open a new shell, or `source` the env file the installer prints (`~/.local/bin/env` on a typical install).

[gitleaks](https://github.com/gitleaks/gitleaks) (required for the pre-commit secret scan). Pick one:

```bash
# Homebrew on Linux
brew install gitleaks

# Debian/Ubuntu (package version may lag)
sudo apt install -y gitleaks
```

Or install a release binary from https://github.com/gitleaks/gitleaks/releases and put `gitleaks` on `PATH`.
...
- **`uv: command not found`** — open a new shell, or add `$HOME/.local/bin` to `PATH`.
